...
- **Board view** — 7 pipeline-stage columns (New Lead → Contacted →
  Assessment Scheduled → Assessment Outcome → Onboarding → Active Client →
  Program Review). Click a card to open the full client profile.
- **Needs Action Today** — a flat, urgency-sorted list pulled from every
